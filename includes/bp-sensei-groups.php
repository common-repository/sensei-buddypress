	...
	class BuddyPress_Sensei_Groups {
		...
		public function setup() {
			...
			if ( $woothemes_sensei ) {
				...
				add_action( 'groups_before_delete_group', array( $this, 'detach_course_from_group' ), 10, 1 );
			}
		}
		...
		/**
		 * Delete meta_key bp_course_group from course meta before group delete
		 * @param $group_id
		 */
		public function detach_course_from_group( $group_id ) {
			global $wpdb;

			$wpdb->delete(
				$wpdb->postmeta,
				array(
					'meta_key' => 'bp_course_group',
					'meta_value' => $group_id
				),
				array(
					'%s',
